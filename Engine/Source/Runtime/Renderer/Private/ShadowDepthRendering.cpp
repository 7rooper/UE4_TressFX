...
/*@BEGIN third party code TressFX */
void FProjectedShadowInfo::TressFXRenderDeepOpacityMap(FRHICommandListImmediate& RHICmdList, class FSceneRenderer* SceneRenderer, FBeginShadowRenderPassFunction BeginShadowRenderPass)
{
...
//	QUICK_SCOPE_CYCLE_COUNTER(STAT_RenderShadowDepth);
	...
	//}
	{
		// We must have already opened the renderpass by the time we get here.
		check(RHICmdList.IsInsideRenderPass());
		TressFXDeepOpacityPass.DispatchDraw(nullptr, RHICmdList);
...
void GetTressFXDeepOpacityShaders(
	const FMaterial& Material,
	const FVertexFactory* VertexFactory,
	ERHIFeatureLevel::Type FeatureLevel,
	bool bDirectionalLight,
...
)
{
...

	{
		if (bUsePerspectiveCorrectShadowDepths)
		{
			PixelShader = (FTressFXDeepOpacityPS<TFXPixelShadowDepth_PerspectiveCorrect> *)Material.GetShader<FTressFXDeepOpacityPS<TFXPixelShadowDepth_PerspectiveCorrect> >(VFType);
		}
		else if (bOnePassPointLightShadow)
		{
			PixelShader = (FTressFXDeepOpacityPS<TFXPixelShadowDepth_OnePassPointLight> *)Material.GetShader<FTressFXDeepOpacityPS<TFXPixelShadowDepth_OnePassPointLight> >(VFType);
		}
		else
		{
			PixelShader = (FTressFXDeepOpacityPS<TFXPixelShadowDepth_NonPerspectiveCorrect> *)Material.GetShader<FTressFXDeepOpacityPS<TFXPixelShadowDepth_NonPerspectiveCorrect> >(VFType);
		}
	}
}
...
void FTressFXDeepOpacityPassProcessor::Process(
	const FMeshBatch& RESTRICT MeshBatch,
	uint64 BatchElementMask,
	int32 StaticMeshId,
	const FPrimitiveSceneProxy* RESTRICT PrimitiveSceneProxy,
	...
	ERasterizerCullMode MeshCullMode)
{
	...
	TMeshProcessorShaders<
		...
		FBaseDS,
		FMeshMaterialShader
	> TressFXDeepOpacityPassShaders;

	const bool bUsePositionOnlyVS = VertexFactory->SupportsPositionOnlyStream()
		&& MaterialResource.WritesEveryPixel(true)
		&& !MaterialResource.MaterialModifiesMeshPosition_RenderThread();

	GetTressFXDeepOpacityShaders(
		MaterialResource,
		VertexFactory,
		FeatureLevel,
		ShadowDepthType.bDirectionalLight,
		ShadowDepthType.bOnePassPointLightShadow,
		bUsePositionOnlyVS,
		TressFXDeepOpacityPassShaders.VertexShader,
		TressFXDeepOpacityPassShaders.HullShader,
		TressFXDeepOpacityPassShaders.DomainShader,
		TressFXDeepOpacityPassShaders.PixelShader);

	FShadowDepthShaderElementData ShaderElementData;
	ShaderElementData.InitializeMeshMaterialData(ViewIfDynamicMeshCommand, PrimitiveSceneProxy, MeshBatch, StaticMeshId, false);

	const FMeshDrawCommandSortKey SortKey = CalculateMeshStaticSortKey(TressFXDeepOpacityPassShaders.VertexShader, TressFXDeepOpacityPassShaders.PixelShader);
...
void FTressFXDeepOpacityPassProcessor::AddMeshBatch(const FMeshBatch& RESTRICT MeshBatch, uint64 BatchElementMask, const FPrimitiveSceneProxy* RESTRICT PrimitiveSceneProxy, int32 StaticMeshId)
{

	const FMaterialRenderProxy* FallbackMaterialRenderProxyPtr = nullptr;
	const FMaterial& Material = MeshBatch.MaterialRenderProxy->GetMaterialWithFallback(FeatureLevel, FallbackMaterialRenderProxyPtr);

	if (MeshBatch.CastShadow && Material.IsUsedWithTressFX() && MeshBatch.bTressFX)
	{
		const FMaterialRenderProxy& MaterialRenderProxy = FallbackMaterialRenderProxyPtr ? *FallbackMaterialRenderProxyPtr : *MeshBatch.MaterialRenderProxy;
		const EBlendMode BlendMode = Material.GetBlendMode();
		const EMaterialShadingModel ShadingModel = Material.GetShadingModel();
		const bool bReflectiveShadowmap = false;
		const bool bShouldCastShadow = Material.ShouldCastDynamicShadows();
...
FTressFXDeepOpacityPassProcessor::FTressFXDeepOpacityPassProcessor(
	const FScene* Scene,
	const FSceneView* InViewIfDynamicMeshCommand,
	const TUniformBufferRef<FViewUniformShaderParameters>& InViewUniformBuffer,
	FUniformBufferRHIParamRef InPassUniformBuffer,
...
FRegisterPassProcessorCreateFunction RegisterTressFXDeepOpacityPass(&CreateTressFXDeepOpacityPassProcessor, EShadingPath::Deferred, EMeshPass::TressFX_DeepOpacity, EMeshPassFlags::CachedMeshCommands);
/*@END third party code TressFX */