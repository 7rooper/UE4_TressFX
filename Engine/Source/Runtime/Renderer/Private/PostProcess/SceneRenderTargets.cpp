...
{	
	...
	if (bUseGBuffer)
	{
		...
		// Create the specular color and power g-buffer.
		{
			...
			FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(BufferSize, SpecularGBufferFormat, FClearValueBinding::Transparent, TexCreate_None, TexCreate_RenderTargetable /*@BEGIN Third party code TressFX*/ | TexCreate_UAV | TexCreate_ShaderResource /*@END Third party code TressFX*/ , false));
...
void FSceneRenderTargets::AllocateTressFXScreenShadowMask(FRHICommandList& RHICmdList, TRefCountPtr<IPooledRenderTarget>& TressFXScreenShadowMaskTexture)
{
	...
	FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(GetBufferSizeXY(), PF_B8G8R8A8, FClearValueBinding::White, TexCreate_None, TexCreate_RenderTargetable, false));
	//Desc.Flags |= GFastVRamConfig.ScreenSpaceShadowMask; //jaketodo, test if i can use the same config here
	Desc.NumSamples = GetNumSceneColorMSAASamples(GetCurrentFeatureLevel());
